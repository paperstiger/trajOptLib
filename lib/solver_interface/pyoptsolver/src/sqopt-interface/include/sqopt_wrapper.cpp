...
public:
    ...
    void solve(int type)
    {
        ...
        char solveType[20];
        if (type == 0)
            memcpy(solveType, "Cold", strlen("Cold"));
        else if (type == 1)
            memcpy(solveType, "Warm", strlen("Warm"));
        else
            memcpy(solveType, "Hot", strlen("Hot"));

        SqoptEZ::solve(solveType);
#else
        if (type == 0)
            SqoptEZ::solve("Cold");
        else
...
PYBIND11_MODULE(libsqopt, m) {
    ...
    .def("solve", &PySqopt::solve, R"pbdoc(
            ...
            Args:
                type (int): 0 for cold start and 1 for warm start.
        )pbdoc")
