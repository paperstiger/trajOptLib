/* ./src/snoptc.f -- translated by f2c (version 20100827).
	...
	ftnlen names_len, ftnlen cu_len, ftnlen cw_len)
{
    ...
    static integer negcon, lgsave, nmajor;
...
/*     s8Map does not allocate space for x0, gsave, so it is done here. */
/*     ------------------------------------------------------------------ */
...
/*     It is now safe to copy x to x0 */
    if (fponly) {
